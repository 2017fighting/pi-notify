/**
 * @raincore/pi-notify — Command registration
 *
 * Registers slash commands for notification configuration and testing.
 */

import type { ExtensionAPI, ExtensionContext } from "@earendil-works/pi-coding-agent";
import { UNIPI_PREFIX } from "./vendored/index.js";
import { NOTIFY_COMMANDS } from "./vendored/index.js";
...
import type { CachedModel } from "./vendored/index.js";
...
export function registerNotifyCommands(pi: ExtensionAPI): void {
  // /notify:settings — Opens settings TUI overlay
  pi.registerCommand(
  ...
  // /notify:recap-model — Open recap model selector directly
  pi.registerCommand(
  ...
  // /notify:set-gotify — Interactive Gotify setup
  pi.registerCommand(
  ...
  // /notify:set-tg — Interactive Telegram setup
  pi.registerCommand(
  ...
  // /notify:set-ntfy — Interactive ntfy setup
  pi.registerCommand(
  ...
  // /notify:set-bark — Interactive Bark setup
  pi.registerCommand(
    `${UNIPI_PREFIX}${NOTIFY_COMMANDS.SET_BARK}`,
    {
      description: "Set up Bark push notifications with connection test",
      handler: async (_args: string, ctx: ExtensionContext) => {
        if (!ctx.hasUI) {
          ctx.ui.notify("Bark setup requires an interactive UI.", "warning");
          return;
        }
  ...
  // /notify:event <event> <on|off> — Non-TUI event toggle (issue #27 escape
    ...
    `${UNIPI_PREFIX}${NOTIFY_COMMANDS.NOTIFY_EVENT}`,
    {
      ...
      handler: async (args: string, ctx: ExtensionContext) => {
        ...
        if (parts.length !== 2 || (value !== "on" && value !== "off")) {
          ctx.ui.notify(
            `Usage: /${UNIPI_PREFIX}${NOTIFY_COMMANDS.NOTIFY_EVENT} <event> <on|off>`,
            "warning"
          );
          return;
        }
  ...
  pi.registerCommand(
    `${UNIPI_PREFIX}${NOTIFY_COMMANDS.TEST}`,
    {
      ...
      handler: async (_args: string, ctx: ExtensionContext) => {
        ...
        // Bark — resolved from project/global bark.json
        const barkConfig = loadBarkConfig(process.cwd());
        if (barkConfig.enabled && barkConfig.serverUrl && barkConfig.deviceKey) {
          try {
            ...
            results.push("✓ Bark: sent");
          } catch (err) {
            results.push(
              `✗ Bark: ${err instanceof Error ? err.message : "failed"}`
            );
          }
        }

        if (results.length === 0) {
          ctx.ui.notify(`No platforms enabled. Use /${UNIPI_PREFIX}${NOTIFY_COMMANDS.SETTINGS} first.`, "warning");
        } else {
