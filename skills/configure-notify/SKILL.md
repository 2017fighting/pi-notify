...
Help users configure the `@raincore/pi-notify` notification system.
...
**Main config (platforms + events):** `~/.pi/agent/notify/config.json`

**ntfy config (dedicated file):**
- Global: `~/.pi/agent/notify/ntfy.json`
- Project: `<project>/.pi/agent/notify/ntfy.json`
...
TUI: `/notify:settings` → Platforms → Quiet after activity (Space), ←→ then Space for channels, +/− for the window (1s steps).
...
TUI: `/notify:settings` → Re-notify → Space toggles enabled, +/− adjusts interval (30s steps) and max repeats.
...
**Setup options:**
1. **Interactive overlay:** Tell user to run `/notify:set-gotify` for guided setup with connection test
...
- `chatId` — Auto-detected by `/notify:set-tg`
...
**Setup options:**
1. **Interactive overlay:** Run `/notify:set-ntfy` for guided setup with scope selection and connection test
...
**File locations:**
- Global: `~/.pi/agent/notify/ntfy.json` (all projects)
- Project: `<project>/.pi/agent/notify/ntfy.json` (this project only)
...
**Scope selection in wizard:** When running `/notify:set-ntfy`, the wizard now asks where to save the config (Global or Project). Re-running the wizard pre-selects the current scope and pre-fills existing values.

**Settings overlay:** The ntfy line in `/notify:settings` shows topic, priority, and scope label (`[project]`, `[global]`, or "Not configured").
...
### Bark (default: disabled)

iOS push notifications via the official [Bark](https://github.com/Finb/Bark) app — api.day.app (default) or a self-hosted bark-server.
Requires:
- `serverUrl` — Bark server URL (default: `https://api.day.app`)
- `deviceKey` — Device key from the Bark app (the path segment of the URL shown there)
- `group` — Optional message group (same-group messages fold together)
- `sound` — Optional alert sound name
- `level` — Optional interruption level: `active` | `passive` | `timeSensitive` | `critical` (camelCase; `critical` needs the in-app entitlement). Wins over the semantic priority mapping when set.
- `icon` — Optional notification icon URL (remote URL only; defaults to the Pi badge)
- `timeoutMs` — HTTP timeout (default: 4000)

**Setup options:**
1. **Interactive overlay:** Run `/notify:set-bark` for guided setup with scope selection and connection test
2. **Manual config:** Edit `bark.json` directly (same resolution as ntfy: project `bark.json` → global `bark.json` → disabled)
3. **Agent can write config:** Read the current bark.json, merge changes, write back

**File locations:**
- Global: `~/.pi/agent/notify/bark.json` (all projects)
- Project: `<project>/.pi/agent/notify/bark.json` (this project only)
...
Keep `bark.json` private — the device key is in it. Semantic priority maps low/normal/high → passive/active/timeSensitive; plain dispatches send no level (Bark defaults to `active`).

## Commands

| Command | Description |
|---------|-------------|
| `/notify:settings` | TUI overlay to toggle platforms and events |
| `/notify:set-gotify` | Interactive Gotify setup wizard |
| `/notify:set-tg` | Interactive Telegram setup wizard |
| `/notify:set-ntfy` | Interactive ntfy setup wizard |
| `/notify:set-bark` | Interactive Bark setup wizard |
...
```bash
cat ~/.pi/agent/notify/config.json
```
...
For Gotify: suggest running `/notify:set-gotify`
For Telegram: suggest running `/notify:set-tg`
For ntfy: suggest running `/notify:set-ntfy`
For general settings: suggest `/notify:settings`
