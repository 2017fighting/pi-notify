...
function readNotifyConfig(): Record<string, unknown> {
  const p = join(home, ".pi/agent/notify/config.json");
...
function seedBaselineSelection(): void {
  const dir = join(home, ".pi/agent/notify");
...
describe("settings overlay: toggle, tab, model-selector key, save", () => {
  ...
  it("saves with Enter in both encodings", () => {
    for (const enc of ENTER_ENCODINGS) {
      ...
      assert.ok(
        existsSync(join(home, ".pi/agent/notify/config.json")),
...
function goToSilenceChips(overlay: NotifySettingsOverlay): void {
  for (let i = 0; i < 7; i++) overlay.handleInput("j");
}
...
describe("settings overlay: silence after input", () => {
  ...
  it("saves all five chips as an empty platforms array", () => {
    freshHome();
    const overlay = new NotifySettingsOverlay();
    goToSilenceChips(overlay);
    overlay.handleInput("l");
