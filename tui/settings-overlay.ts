/**
 * @raincore/pi-notify — Settings TUI Component
 *
...
import { loadBarkConfig, saveBarkConfig, getBarkConfigScope } from "../bark-config.js";
import type { NotifyConfig, NotifyPlatform, NtfyConfig, BarkConfig } from "../types.js";
import { OverlayTheme, boxInnerWidth } from "../vendored/index.js";

/** Section types */
type Section = "platforms" | "events" | "recap" | "renotify";

const PLATFORM_KEYS: NotifyPlatform[] = ["native", "gotify", "telegram", "ntfy", "bark"];
...
const SUPPRESS_FOCUSED_INDEX = 5;
const SILENCE_MASTER_INDEX = 6;
const SILENCE_CHIPS_INDEX = 7;
...
export class NotifySettingsOverlay implements Component {
  ...
  private barkConfig: BarkConfig;
  private barkScope: "project" | "global" | "none";
  ...
  constructor() {
    ...
    this.barkConfig = loadBarkConfig(cwd);
    this.barkScope = getBarkConfigScope(cwd);
  }
  ...
  private get maxItems(): number {
    if (this.section === "platforms") return 8; // 5 platforms + focused + silence master + chips
    ...
    if (this.section === "platforms") {
      if (this.selectedIndex < PLATFORM_KEYS.length) {
        ...
        if (key === "ntfy") {
          ...
          // bark toggle updates the resolved bark config
          this.barkConfig.enabled = !this.barkConfig.enabled;
        } else if (key) {
  ...
  private save(): void {
    ...
    // Save Bark config to its own file if scope is known
    if (this.barkScope !== "none") {
      saveBarkConfig(this.barkScope, process.cwd(), this.barkConfig);
    }
    this.saved = true;
  ...
  private renderPlatforms(lines: string[], innerWidth: number): void {
    const platforms: Array<{
      ...
      },
      {
        key: "bark",
        label: "Bark",
        detail: this.barkScope !== "none"
          ? `Server: ${this.barkConfig.serverUrl ?? "—"} · [${this.barkScope}]`
          : "Not configured (iOS push)",
      },
    ];
    ...
    for (let i = 0; i < platforms.length; i++) {
      ...
      // ntfy/bark enabled state comes from their resolved .json files, not config.json
      const isEnabled =
        p.key === "ntfy" ? this.ntfyConfig.enabled
        : p.key === "bark" ? this.barkConfig.enabled
        : this.config[p.key].enabled;
